...
public class SS extends JFrame {
    ...
    CustomQueue<Task> queueSystem = new CustomQueue<>();
    CustomLinkedList<Task> LinkedListSystem = new CustomLinkedList<>();
    CustomStack<Task> StackSystem = new CustomStack<>();
    public SS() {
    ...
    private void initializeUI() {
        ...
        // Read tasks and process them

        readTasksFromFile(queueSystem);
        readTasksFromFile(LinkedListSystem);
        readTasksFromFile(StackSystem);
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(filepath))) {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if (parts.length >= 3) {
                    ...
                    System.out.println(task.getTimeOfArrival());

                    // Add the Task to the customQueue
                    dataStructure.add(task);
                    //Thread.sleep(1);
                }
            }
            processTasks(dataStructure, dataStructure.getName());
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    private void processTasks(CustomDataStructure<Task> schedulingSystem, String systemName) {
        Task task;
        long total_response_time = 0;
        long total_turnaround_time = 0;
        int total_tasks = schedulingSystem.getSize();
        ...
        while ((task = schedulingSystem.remove()) != null) {
            ...
            tableModel.addRow(new Object[]{
                    ...
                    formatTime(turnaroundTime) // Turnaround time equals response time in this example
            });
            total_response_time += responseTimeNanos;
            total_turnaround_time += turnaroundTimeNanos;
        }
        */
/*ZonedDateTime SystemEndTime = Instant.now().atZone(malaysiaTimeZone);
        String message = "Program Start Time: " + formatter.format(SystemStartTime) +
                "\nProgram End Time: " + formatter.format(SystemEndTime) +
                "\nAverage Response Time: " + formatTime(total_response_time / total_tasks) +
                "\nAverage Turnaround Time: " + formatTime(total_turnaround_time / total_tasks);
        JOptionPane.showMessageDialog(this, message, "Task Statistics", JOptionPane.INFORMATION_MESSAGE);*//*

        displaySystemStatistics(systemName, total_response_time, total_turnaround_time, total_tasks);
    }
    ...
    private void displaySystemStatistics(String systemName, long totalResponseTime, long totalTurnaroundTime, int totalTasks) {
        ZonedDateTime systemEndTime = Instant.now().atZone(malaysiaTimeZone);
        String message = String.format("%s Statistics:\n" +
                        "Average Response Time: %s\n" +
                        "Average Turnaround Time: %s",
                systemName,
                formatTime(totalResponseTime / totalTasks),
                formatTime(totalTurnaroundTime / totalTasks));
